...
SELECT LAST_NAME,SALARY ,SALARY * 15 //연산된 부분은 새로운 컬럼이 된다
FROM EMPLOYEES;

--컬럼의 별칭(ALIAS)사용

SELECT LAST_NAME 이름,SALARY AS 급여,SALARY * 15 "연 봉"//일부러 공백주는 일 없음
FROM EMPLOYEES;
...
SELECT LAST_NAME 사원이름 || '사원의 급여는' || SALARY 급여 || '입니다.'
FROM EMPLOYEES;
...
--100번 부서가 아닌 사원들만 출력합니다
SELECT*
FORM EMPLOYEES
WHERE DEPARTMENT_ID !=100;
//문자열끼리 비교할때는 꼭 작은따옴표를 써라

SELECT*
FROM EMPLOYEES
WHERE LAST_NAME = 'King'; //대소문자도 비교하니 조심

--2)논리연산자
--사원의 부서가 100번 부서이면서 급여가 10000 이상인 사원의 정보를 출력
SELECT*
FROM EMPLOYEES
WHERE DEPARTMENT_ID = 100 AND SALARY >= 10000;

--사원의 부서가 100번 또는 90번인 사원의 정보를 출력
SELECT*
FROM EMPLOYEES
WHERE DEPARTMENT_ID = 90 OR DEPARTMENT_ID = 100 ;

--사원의 부서가 100번 또는 90번이면서 급여가 10000이상인 사원의 정보를 출력
SELECT*
FROM EMPLOYEES
WHERE ( DEPARTMENT_ID = 90 OR DEPARTMENT_ID = 100 ) AND SALARY >= 10000;//괄호 없으면 OR 부터 우선순위가 높은 AND부터 연산됨

--급여가 10000이상인 사원의 수
SELECT COUNT(*) //조건에 해당하는 수만큼 숫자를 반환
FROM EMPLOYEES
WHERE SALARY >= 10000;